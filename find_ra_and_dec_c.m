% ~~~~~~~~~~~~~~~~~~~~~~
function [cra,cdec,cpath] = find_ra_and_dec_c(to,tf,orbittime,numsamp,numsamp_s,T,e,h,mu,Wo,Wdot,wpo,wpdot,incl,we)
% ~~~~~~~~~~~~~~~~~~~~~~
...
cra = nan(1,numsamp_s*orbittime);
cdec = nan(1,numsamp_s*orbittime);
cpath = nan(1,numsamp_s*orbittime);
...
for i = 1:length(times)
    ...
    % Find Right Ascension and Declination (Cone)
    [conealpha, conedelta, clen] = cone_ra_and_dec_from_r(r_rel,v_rel,pi/4,110,i,numsamp);
    
    % Store Data for Output
    cra(i) = conealpha;          % Cone
    cdec(i) = conedelta;         % Cone
    cpath(i) = clen;
end
